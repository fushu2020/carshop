...
<div class="list w">
    <el-row>
            <el-divider content-position="left" class="divider">所有SUV</el-divider>
  <el-col :span="8" v-for="(list,index) in carList" :key="index" :offset="index > 0 ? 2 : 0" class="list-card">
    <el-card :body-style="{ padding: '0px' }" class="card">
      <img :src="list.mainUrl" class="image">
...
export default {
    ...
    methods: {
      ...
      getList(){//从后台获取SUV的列表数据
        let service = gengerous.url
        axios.get(service+'/car/selectCar?modelId=2',{}).then(res=>{
      ...
      getPage(){//获取分页
        let service = gengerous.url
        axios.get(service+'/car/selectCar?pageSize=12&modelId=2&pageNum='+this.pageNum,{}).then(res=>{
        console.log(res.data);
        this.carList=res.data.data.dataList;
