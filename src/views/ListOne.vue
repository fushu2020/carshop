...
export default {
    ...
    methods: {
      ...
      getList(){//从后台获取轿车的列表数据
        let service = gengerous.url
        axios.get(service+'/car/selectCar?modelId=1',{}).then(res=>{
      ...
      getPage(){//获取分页
        let service = gengerous.url
        axios.get(service+'/car/selectCar?pageSize=12&modelId=1&pageNum='+this.pageNum,{}).then(res=>{
        console.log(res.data);
        this.carList=res.data.data.dataList;
