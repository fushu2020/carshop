...
<div class="list w">
    <el-row>
            <el-divider content-position="left" class="divider">所有跑车</el-divider>
...
export default {
    ...
    methods: {
      ...
      getList(){//从后台获取跑车的列表数据
        let service = gengerous.url
        axios.get(service+'/car/selectCar?modelId=3',{}).then(res=>{
      ...
      getPage(){//获取分页
        let service = gengerous.url
        axios.get(service+'/car/selectCar?pageSize=12&modelId=3&pageNum='+this.pageNum,{}).then(res=>{
        console.log(res.data);
        this.carList=res.data.data.dataList;
